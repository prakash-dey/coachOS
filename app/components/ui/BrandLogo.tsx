...
export function BrandMark({ className }: Readonly<{ className?: string }>) {
  return (
    <span aria-hidden="true" className={cn("grid size-10 shrink-0 place-items-center rounded-xl bg-white p-1 shadow-sm", className)}>
      <Image src="/brand/coachos-logo.png" alt="" width={40} height={40} className="size-full object-contain" priority />
    </span>
  );
}

export function BrandLogo({ href = "/", inverse = false, className }: Readonly<{ href?: string; inverse?: boolean; className?: string }>) {
  return (
