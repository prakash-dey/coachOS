...
export default async function CheckInsPage({
  ...
  const { data: checkIns, error: checkInsError } = await supabase
    .from("check_ins")
    .select(
      "id, week_start, weight_kg, energy_score, mood_score, notes, coach_feedback, submitted_at",
    )
    .eq("workspace_id", membership.workspace_id)
    ...
    <main className="min-h-screen px-6 py-10">
      <section className="mx-auto max-w-3xl">
        ...
        <header className="mt-6 flex items-start justify-between gap-4">
          <div>
            <h1 className="text-3xl font-semibold">Weekly check-ins</h1>
        ...
        {checkIns.length === 0 ? (
          <div className="mt-10 rounded-lg border border-dashed border-gray-300 p-10 text-center">
            ...
            <p className="mt-2 text-sm text-gray-600">
              Submit your first weekly check-in to begin tracking progress.
            </p>
          </div>
        ) : (
