...
export function WorkspaceSwitcher({ workspaces, selectedId, canCreate = false, inverse = false }: WorkspaceSwitcherProps) {
  ...
  return <div ref={rootRef} className="relative min-w-0">
    ...
    {open && <div id={menuId} role="menu" aria-label="Choose a workspace" className="absolute left-0 z-50 mt-2 w-[min(20rem,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-border bg-surface p-2 text-foreground shadow-[0_20px_60px_rgba(30,22,60,0.22)]">
