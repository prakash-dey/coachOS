...
export async function createWorkspace(formData: FormData) {
  ...
  if (workspaceName.length < 1 || workspaceName.length > 120) redirect("/onboarding?error=invalid_workspace");
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect("/login");
  const { data: workspaceId, error } = await supabase.rpc("create_coach_workspace", { workspace_name: workspaceName });
  if (error || !workspaceId) redirect(`/onboarding?error=${error?.code === "23514" ? "workspace_limit" : "workspace_failed"}`);
